...
#         self.root = root
#         self.root.title("Mini Edu - Login")
...
#         self.add_background()
#         frame = self.create_login_frame()
#         self.add_logo(frame)
#         self.add_heading(frame)
#         self.password = self.add_password_entry(frame)
#         self.add_login_button(frame)

#     def add_background(self):
#         img = PhotoImage(file="images/bg_login.png")
#         Label(self.root, image=img, bg="white").place(x=50, y=50)
#         self.bg_image = img  # Giữ tham chiếu để tránh bị xóa khỏi bộ nhớ
...
#         logo_image = ImageTk.PhotoImage(Image.open(logo_path).resize((50, 50)))
#         Label(frame, image=logo_image, bg="white").place(x=20, y=10)
#         self.logo_image = logo_image  # Giữ tham chiếu

#     def add_heading(self, frame):
#         Label(frame, text="Mini Edu System", font=("Arial", 20, "bold"), bg="white", fg="#1C2442").place(x=80, y=15)

#     def add_password_entry(self, frame):
#         password = Entry(frame, width=25, fg='#1C2442', border=0, bg="white", font=("Arial", 11))
#         password.place(x=30, y=150)
#         password.insert(0, 'Password')
#         Frame(frame, width=262, height=2, bg="#1C2442").place(x=30, y=175)
...
#         def login():
#             if self.password.get() == "123":  # Giả định mật khẩu là "123"
#                 messagebox.showinfo("Login", "Đăng nhập thành công!")
#                 self.root.destroy()
#                 subprocess.run(["python", "gui/home_page.py"])  # Chuyển đến trang mới
#             else:
